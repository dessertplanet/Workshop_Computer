...
## 18. Norns Compatibility Layer (Experimental)

This emulator now exposes a minimal compatibility surface so unmodified norns `norns.crow` host code can interact with it over the standard line protocol.

### Implemented Packets (Device → Host)

| Packet | When Emitted | Notes |
|--------|--------------|-------|
| `^^ready()` | On power-up hello, after script upload (`^^e`/`^^w`), after `crow.reset()`, after loading First.lua / flash script | Mirrors crow readiness semantics. |
| `^^version('x.y.z')` | On hello, on `^^v` | Static semantic version string (firmware controlled). |
| `^^identity('0xXXXXXXXXXXXXXXX')` | On hello, on `^^i` | 64‑bit unique ID (hex). |
| `^^output(ch,volts)` | On changed Lua output voltage (telemetry) | Debug / inspection aid. |
| `^^stream(ch,volts)` | Periodic / delta-limited input telemetry | For druid / monitoring tools. |
| `^^pub(name,val,typ)` | During public param declaration & discover cycle | `typ` is a (possibly empty) table. |
| `^^pub('_end')` | End of discover listing | Signals host to finalize population. |
| `^^pupdate(name,val[,sub])` | When public param changes | `sub` used for list index updates (future). |
| `^^pubview(domain,ch,val)` | On `public.view_*` calls | Mirrors viewing telemetry from scripts. |
| Arbitrary `^^EVENT(...)` | From user script `tell(event, ...)` | Host dispatches via dynamic event mapping. |

### Host → Device Commands Recognized

| Command | Action |
|---------|--------|
| `^^s` | Begin script upload (lines buffered until `^^e`/`^^w`). |
| `^^e` | End upload, execute (ephemeral). |
| `^^w` | End upload, persist to flash then execute. |
| `^^v` | Request version (emits `^^version`). |
| `^^i` | Request identity (emits `^^identity`). |
| `^^k` | Kill / re-init Lua VM (emits `^^ready()` after reinit). |
| `^^c` | Clear persisted user script. |
| `^^f` / `^^F` | Load `First.lua` (persisted or default) + ready. |
| Raw Lua line | Evaluated in the single global Lua environment. |
| `` ``` (triple backtick toggle) `` | Multiline REPL mode (same as before). |

### Added Lua Globals (Device Side)

| Global | Purpose |
|--------|---------|
| `tell(ev, ...)` | Serialize args and emit `^^ev(...)` line. |
| `public.add(name,val[,typ])` | Declare a public parameter (immediately emits `^^pub`). |
| `public.update(name,val[,sub])` | Emit `^^pupdate` & mutate stored value. |
| `public.discover()` | Re-emit all declarations followed by `^^pub('_end')`. |
| `public.view_input(ch,val)` / `public.view_output(ch,val)` | Emit `^^pubview`. |
| Internal minimal `_q()` | Lightweight serializer (numbers, strings, tables with primitive keys). |

### Serialization Notes

- Numbers: 6 significant digits (`%.6g`).
- Strings: Standard Lua `%q` quoting (handles escapes).
- Tables: `{[key]=value,...}` unsorted hash order (sufficient for host parsing).
- No cycle detection; user must avoid self-referential structures.

### Public Parameter Typing

`typ` is carried verbatim as the 3rd argument in `^^pub`. Patterns compatible with norns logic:
- Numeric range: `{min,max,"float"}` or `{min,max,"int"}`.
- Option set: `{"option","a","b","c"}`.
- Readonly future: (planned) leading `'@'` not yet generated by emulator.
- Empty `{}` allowed (host infers type).

### Typical Upload / Discover Flow

1. Host sends `^^s`, then script lines, then `^^e`.
2. Emulator:
   - Loads & runs script.
   - Script executes `public.add(...)` declarations (emitting `^^pub` per call).
   - Script finishes `init()`; emulator emits `^^ready()` (after successful load).
   - Host may trigger rediscover by calling `public.discover()` remotely if needed.

### Example Minimal Public Script (host perspective)

```
^^s
public.add("gain",0.5,{0,1,"float"})
public.add("mode",{"a","b","c"},{"option","a","b","c"})
function init() print("ok") end
^^e
```

Expected line sequence (ordering of `pub` lines matches declaration order):
```
^^version('0.1.0')
^^identity('0x...')      (initial hello, may precede upload)
^^ready()
script upload started
script loaded successfully
^^pub('gain',0.5,{[1]=0,[2]=1,[3]='float'})
^^pub('mode',{'a','b','c'},{[1]='option',[2]='a',[3]='b',[4]='c'})
^^pub('_end')
^^ready()
```

(Exact table key ordering may differ; host should not rely on order inside braces.)

### Limitations / Differences vs Hardware Crow

| Aspect | Emulator Behavior | Consideration |
|--------|-------------------|---------------|
| Public freeze (`freezescript`) | Not implemented | Use persistent upload (`^^w`). |
| Quoting optimization | Disabled (no length optimization) | Slightly more bytes over USB; negligible on typical bandwidth. |
| Error events | Printed plain text (no `^^error(...)`) | Future enhancement. |
| Event flood protection | None (user responsible) | Avoid tight loops calling `tell()`. |
| Discover auto-trigger | Not automatically re-run after `ready()` | Host can send `public.discover()` if needed. |

### Validation Script

A helper script `test_public.lua` has been added:
- Declares three public params.
- Periodically modulates them via `metro[1]`.
- Emits `^^pupdate` events host can verify.

Upload with the standard `^^s` / `^^e` sequence to confirm host integration.

### Host Integration Checklist

1. Confirm single `^^ready()` after power-on hello.
2. Upload `test_public.lua`; check for ordered `^^pub` lines + `_end`.
3. Observe ongoing `^^pupdate` for `gain`, `mode`, `depth`.
4. Issue `crow.reset()`; expect environment to reinitialize and emit new `^^ready()` (public declarations must be re-sent by script, matching real crow reset semantics).

If all pass, baseline norns protocol compatibility is established.

---

Revision: 0.3.3 - Added Section 18 (Norns Compatibility Layer), implemented `tell` + public param system, test_public.lua reference, clarified ready emission semantics.
