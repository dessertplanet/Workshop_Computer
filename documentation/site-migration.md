...
- **Phase 4 (remove obsolete paths) and author preview/editor: not started.** Note: `src/discover/*` is still the active data pipeline (it orchestrates parse → `model/card.js`), so Phase 4 is now largely a rename/consolidation rather than dead-code removal.
