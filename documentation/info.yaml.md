...
Modes are optional named snapshots of the whole panel reference: diagram labels, controls, switch text, sockets, and LEDs. Use them when those meanings change. Do not declare modes merely because the DSP has internal states if the panel reference remains the same.
...
modes:
  ...
  - id: edit
    name: Edit
    description: Edit the selected sequence step
    state: { z: middle }

controls:
  knobs:
    - main: { name: Tempo }       # shared base definition
    - when: { mode: play }
      x: { name: Sequence Length }
      y: { name: Pitch Offset }
    - when: { mode: edit }
      x: { name: Step Pitch }
      y: { name: Step Velocity }
```
...
| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `id` | yes | string | Stable lowercase kebab-case identifier used by `when.mode`. |
| `name` | yes | string | Human-readable combo-box label. |
| `description` | no | string | Short explanation shown with that panel snapshot. |
| `default` | no | boolean | Initial snapshot. At most one mode may be default; the first mode is used if none is marked. |
| `state` | no | object | Conditions that are true while the mode is active, such as `{ z: up }`, `{ transport: record }`, or `{ usb-power: grid }`. |
| `activate` | no | object or object[] | One or more actions that enter or toggle the mode. This is descriptive metadata and may use arbitrary keys. |

`state` describes an active mode. `activate` describes how the user enters it. For example, a mode active only while Z is held uses `state`, while a persistent layer toggled by holding Z uses `activate`:

```yaml
modes:
  - id: alternate-held
    name: Alternate (held)
    state: { z: down, gesture: hold }

  - id: alternate-latched
    name: Alternate (latched)
    activate:
      - { z: down, gesture: hold, behavior: toggle }
      - { input: PulseIn2, gesture: rising-edge }
```

Conditions may be attached to knob rows, LED rows, panel socket entries, and conditional switch rows. Prefer `when: { mode: id }` for mode-specific metadata. Existing direct conditions such as `when: { z: up }` remain valid and can provide definitions shared by several modes.
...
panel:
  inputs:
    ...
    - id: CVIn1
      when: { mode: edit }
      name: Step Modulation

controls:
  switch:
    - up: { name: Play }
      middle: { name: Edit }
    - when: { mode: calibration }
      up: { name: Increase Value }
      middle: { name: Save Value }
      down: { name: Next Point }
```

The original `controls.switch` object shorthand remains valid when switch meanings do not vary by mode.

Mode resolution starts with unconditional definitions, then applies matching conditional definitions from least to most specific. A condition value of `any` is a wildcard. More-specific rows override less-specific rows; equally specific rows use later-row-wins ordering. Sockets merge by API `id`, knobs by `main` / `x` / `y`, switch definitions by position, and LEDs by LED `id`.

The card details and author preview show a mode combo box only when two or more modes are declared. Cards with no modes retain their existing output and UI.
