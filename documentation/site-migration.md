...
- `tags`
- `panel`
- `panel_modes` (optional resolved panel snapshots; the default also populates `panel`, `switch_modes`, and `leds` for compatibility)
- `switch_modes`
- `leds`
- `documentation`
- `notes`
...
- **Named panel modes: implemented in the schema/model/UI.** Authors may declare optional top-level `modes` and target them with `when.mode`; the model resolves base-plus-override snapshots across knobs, switch text, sockets, and LEDs. Detail and preview pages show a combo box only for cards with multiple modes. Existing direct conditions and cards without modes remain supported. Release metadata adoption is intentionally incremental.
