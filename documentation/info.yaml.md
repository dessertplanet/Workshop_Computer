...
| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `demo-link` | no | string (URL) | Optional demo video URL: YouTube (`watch`, `youtu.be`, `/shorts/`, `/embed/`) or Instagram (`/reel/`, `/p/`, `/tv/`). Rendered as a demo-video thumbnail on the detail page that plays inline when clicked. Matching YouTube and Instagram links in README.md also get inline embeds (Instagram embeds use a portrait frame). |
