...
- **Panel views: implemented.** Without a `panels/` directory, unconditioned controls, sockets, and LEDs form a shared base and `when.z` Up/Middle/Down rows produce generated physical-position views. A `panels/manifest.yaml` instead supplies N ordered custom image/Markdown presentations and replaces generated views; `when.panel` attaches structured metadata to their IDs. Legacy `panel`, `switch_modes`, and `leds` retain the generated default snapshot. `controls.switch.tap` remains an action and never creates a generated view.
