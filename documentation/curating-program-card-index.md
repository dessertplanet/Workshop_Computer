...
The GitHub Pages workflow synchronizes missing empty assignments before checking and building. After a card is merged to `main`, the repository-index workflow commits the synchronized `flairs.yml` together with any generated release-index update. Invalid references, stale assignments, unknown flairs, and malformed curation still fail with an actionable error.
